...
class ProfilePage extends ConsumerWidget {
  ...
  @override Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(appBar:AppBar(title:const Text('Profile')),body:ListView(padding:const EdgeInsets.all(20),children:[
      ...
      const SizedBox(height:24),FilledButton(onPressed:()async{await ref.read(clinicRepositoryProvider).signOut();ref.invalidate(currentUserProvider);if(!context.mounted)return;context.go('/login');},child:const Text('Sign out')),
    ]));
  }
}
